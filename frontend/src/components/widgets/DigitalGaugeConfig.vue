  ...
  <div class="flex flex-col gap-5">
    ...
    <div class="p-4 bg-base-200/50 rounded-box border border-base-200">
      ...
      <label class="form-control w-full">
        ...
        <input type="text" v-model="localConfig.unit"
          class="input input-bordered input-sm w-full" placeholder="e.g. °C, %" />
      </label>
    </div>

    <!-- Toggle for Dynamic Color Zones -->
    <div class="p-4 bg-base-200/50 rounded-box border border-base-200">
      <h4 class="font-bold text-sm mb-3 text-base-content">{{ $t('digitalGauge.config.ringColorSettings') }}</h4>
      
      <label class="cursor-pointer label justify-start gap-4">
        <input type="checkbox" v-model="localConfig.useGrades" class="toggle toggle-primary toggle-sm" />
        <span class="label-text font-semibold">{{ $t('digitalGauge.config.enableDynamicZones') }}</span>
      </label>

      <!-- Default color picker if dynamic zones are OFF -->
      <label v-if="!localConfig.useGrades" class="form-control w-full mt-4">
        <div class="label pb-1"><span class="label-text font-semibold">{{ $t('digitalGauge.config.defaultRingColor') }}</span></div>
        <input type="color" v-model="localConfig.defaultRingColor"
    ...
    <div v-if="localConfig.useGrades" class="p-4 bg-base-200/50 rounded-box border border-base-200">
      <div class="flex justify-between items-center mb-4">
        <h4 class="font-bold text-sm text-base-content m-0">{{ $t('digitalGauge.config.dynamicColorZones') }}</h4>
      ...
      <div class="flex flex-col gap-3">
        <div v-for="(grade, index) in localConfig.grades" :key="index" class="flex flex-wrap md:flex-nowrap items-center gap-2 bg-base-100 p-2 rounded border border-base-200">
          ...
          <input type="text" v-model="grade.name" class="input input-bordered input-sm w-full md:w-1/2" :placeholder="$t('digitalGauge.config.zoneName')" />
          ...
          <button @click="removeGrade(index)" class="btn btn-ghost btn-sm btn-square text-error shrink-0" :title="$t('common.delete')">
...
const defaultGrades = computed(() => [
  { name: t('digitalGauge.config.defaults.low'), limit: 25, color: '#58D9F9' },
  { name: t('digitalGauge.config.defaults.medium'), limit: 75, color: '#FDDD60' },
  { name: t('digitalGauge.config.defaults.high'), limit: 100, color: '#FF6E76' }
]);
...
const localConfig = ref({
  ...
  defaultRingColor: props.modelValue.defaultRingColor || '#3b82f6', 
...
const addGrade = () => {
  localConfig.value.grades.push({ 
    name: t('digitalGauge.config.newZone'), 
