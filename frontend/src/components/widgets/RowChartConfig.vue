      ...
      <div class="grid grid-cols-2 gap-4">
        ...
        <label class="form-control w-full">
          <div class="label pb-1">
            <span class="label-text font-semibold">{{ $t('rowChart.config.xAxisLabel') }}</span>
          </div>
          <input type="text" v-model="localConfig.xAxisName"
            class="input input-bordered input-sm w-full" :placeholder="$t('rowChart.config.xAxisPlaceholder')" />
        </label>
      </div>
    </div>
